...
This class is going to represent a human player. In addition to having all of the instance variables and methods that WOFPlayer has, WOFHumanPlayer should have an additional method:

.getMove(category, obscuredPhrase, guessed):
Should ask the user to enter a move (using input()) and returnwhatever string they entered.
...
.SORTED_FREQUENCIES: Should be set to 'ZQXJKVBPYGFWMUCLDRHSNIOATE', which is a list of English characters sorted from
 least frequent ('Z') to most frequent ('E'). We’ll use this when trying to make a “good” move.
...
A higher difficulty should make us more likely to make a “good” move. Implement this by choosing a random number between 1 and 10 using random.randint(1, 10) (see above) and returning False if that random number is greater than self.difficulty. If the random number is less than or equal to self.difficulty, return True.
...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        obscuredPhrase: {self.obscuredPhrase}
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        if self.prizeMoney < VOWEL_COST:
            not_included += VOWELS
        for i in LETTERS:
            if i not in guessed and i not in not_included:
                poss_letters.append(i)
        return poss_letters
